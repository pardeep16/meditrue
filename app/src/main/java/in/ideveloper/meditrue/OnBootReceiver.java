...
public class OnBootReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		if(cursor != null) {
			...
			int titlepos=cursor.getColumnIndex(RemindersDbAdapter.KEY_TITLE);
			while(cursor.isAfterLast() == false) {
				...
				SimpleDateFormat format = new SimpleDateFormat(AddMedicineRemainder.DATE_TIME_FORMAT);

				String titleget=cursor.getString(titlepos);
				
				try {
					java.util.Date date = format.parse(dateTime);
					cal.setTime(date);
					
					reminderMgr.setReminder(rowId, cal,titleget);
